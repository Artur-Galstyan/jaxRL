...
def loss_fn(
    ...
    # baseline = get_baseline_of_trajectory(advantages)
...
def step(
    ...
    loss, grads = eqx.filter_value_and_grad(loss_fn)(
        s_policy, s_states, s_actions, s_rewards, s_gamma
    )
    ...
    for i in tqdm(range(5000)):
        ...
        for batch in dataloader:
            ...
            policy, opt_state, loss = step(
                states, actions, rewards, policy, gamma, optimizer, opt_state
            )
            losses.append(loss)
        all_losses.append(np.mean(losses))
        if (i % 500 == 0 and i >= 100) or i == 5000:
            print(
...
def objective(trial: optuna.Trial, env_name: str):
    ...
    hidden_layers = trial.suggest_int("hidden_layers", 0, 4)
    layers = [obs_space]
    for i in range(hidden_layers):
        layers.append(trial.suggest_int(f"hidden_layer_{i}", 1, 8))
...
def run_trials():
    ...
    database_url = current_file_path.parent / f"studies/{env_name}.db"
    ...
    study.optimize(lambda trial: objective(trial, env_name), n_trials=50, n_jobs=10)
